...
class Cart extends Component {
  ...
  getCartItems = async () => {
    const res = await axios.get(
      `http://localhost:4000/getCartProducts/?id=${this.props.userinfo.userId}`
    );
    console.log(
      `http://localhost:4000/getCartProducts/?${this.props.userinfo.userId}`
    );
      ...
      return (
        <div className='product' key={item.id}>
          ...
          <div className='product-quantity'>
            ...
            <button
              className='pbtn'
              onClick={e => this.IncrementItem(e, item.cartItem.productId)}
            >
              {' '}
              +{' '}
            </button>
          ...
          <div className='product-removal'>
            <button
              ...
              onClick={e => this.remove(e, item.cartItem.productId)}
            >
              Remove
            </button>
          </div>
      ...
      <div className='cart-overall'>
        ...
        <div className='shopping-cart'>
          ...
          <div className='totals'>
            ...
            <div className='totals-item'>
              ...
              <div className='totals-value' id='cart-shipping'>
                {5.0 * this.state.cartItems.length}
              </div>
            </div>
            <div className='totals-item totals-item-total'>
              <label>Grand Total</label>
              <div className='totals-value' id='cart-total'>
                <strong>{sum + sum * 0.05 + 15}</strong>
              </div>
            </div>
          </div>
          ...
          <StripeCheckout
            ...
            token={token =>
              this.handleToken(token, (sum = sum + sum * 0.05 + 15))
            }
